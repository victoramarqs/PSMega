<template>
    <div id="container-paciente">
        <div id="esquerda">
            <img src="../assets/register-patient/esquerda.png" alt="">
        </div>
        <div id="direita">
            <h1 id="title">Editar Novo</h1>
            <h1 id="subtitle">Paciente</h1>

            <!-- <div v-if="error != undefined">
                <p>{{error}}</p>
            </div> -->
            
            <!-- <form action=""> -->
            
            <div>
                <input id="Rectangle90" type="text" placeholder="Nome Completo" v-model="name">
                <img id="icon-name" class="icon" src="../assets/register-patient/iconNome.png" alt="">
            </div>

            <div>
                <input type="text" placeholder="CPF" v-model="cpf" v-mask="'###.###.###-##'" disabled>
                <img id="icon-cpf" class="icon" src="../assets/register-patient/iconCPF.png" alt="">
            </div>

            <div>
                <input type="password" placeholder="Senha" v-model="password">
                <img id="icon-pass" class="icon" src="../assets/login/iconPass.png" alt="">
            </div>

            <div>
                <input type="password" placeholder="Confirmar Senha" v-model="passwordConfirmation">
                <img id="icon-repass" class="icon" src="../assets/login/iconPass.png" alt="">
            </div>

            <button id="Rectangle94" @click="updateConfirmation"><span id="Editar">Salvar</span></button>
            <!-- </form> -->

        </div>
    </div>
</template>

<script>
import axios from 'axios';
export default {
    props:[
        'editarPaciente'
    ],
    created(){

        var req = {
            headers: {
                Authorization: "Bearer " + localStorage.getItem('token')
            }
        }
        
        // this.$route.params.cpf
        axios.get("http://localhost:8686/user/" + this.cpf,req).then(res => {
            console.log(res);

            this.name = res.data.name;
            // this.especialidade = res.data.especialidade;
            this.cpf = res.data.cpf;


        }).catch(err => {
            console.log(err);
            this.$router.push({name: 'Painel'})
        })
    },
    data(){
        return{
            name: '',
            especialidade: '',
            cpf: this.editarPaciente,
            password: '',
            passwordConfirmation: '',
            error: undefined
        }
    },
    methods:{
        updateConfirmation : function(){
            if(this.passwordConfirmation == this.password)
                this.update();
            else
                console.log("As senhas devem ser a mesma!");
        },
        update(){

            var req = {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem('token')
                }
            }

            axios.put("http://localhost:8686/user",{
                name: this.name,
                especialidade: this.especialidade,
                cpf: this.cpf,
                password: this.password
            }, req).then(res => {
                console.log(res);
                this.sucessoEdicao();
            }).catch(err => {
                var msgErro = err.response.data.err;
                console.log(msgErro);

                this.error = msgErro; // // Tratamento de erro em tela para o usuário em vez de console.log.
            })
        },
        sucessoEdicao : function(){
            alert("Paciente editado com sucesso!");
            document.location.reload(true);
        }
    }
}
</script>

<style scoped>
    @import url('https://fonts.googleapis.com/css2?family=Nunito:wght@600;700;800&display=swap');

    #container-paciente {
        width: 800px;
        height: 500px;
        display: flex;

        background: #FFFFFF;
        border-radius: 8px;
    }

    #title {
        position: absolute;
        font-family: Nunito;
        font-style: normal;
        font-weight: 800;
        font-size: 36px;
        line-height: 49px;
        margin-left: 4%;
        margin-top: 1%;

        color: #2E4A7D;
    }

    #subtitle {
        position: absolute;
        margin-top: 3.5%;
        margin-left: 4%;

        font-family: Nunito;
        font-style: normal;
        font-weight: 800;
        font-size: 36px;
        line-height: 49px;
        display: flex;
        align-items: center;

        color: #ff738a;
    }

    #Rectangle90 {
        margin-top: 45%;
    }

    #Rectangle94 {
        width: 300px;
        height: 51px;

        position: relative;
        margin-top: 15%;
        margin-left: 20%;

        background: #2E4A7D;
        border-radius: 16px;
    }

    input {
        width: 300px;
        height: 40px;
        padding-left: 45px;
        margin-top: 5%;
        margin-left: 20%;

        position: relative;
        background: #F5F5F5;
        border: 1px solid #2E4A7D;
        box-sizing: border-box;
        border-radius: 8px;
    }

    .icon{
        position: absolute;
        margin-left: 4.7%;
        margin-top: -2%;
    }

    #Editar {
        font-family: Nunito;
        font-style: normal;
        font-weight: bold;
        font-size: 22px;
        line-height: 30px;
        align-items: center;
        text-align: center;
        color: #FFFFFF;
    }

</style>